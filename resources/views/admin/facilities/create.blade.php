...
<div class="mb-6 flex justify-between items-center">
    <div>
        ...
        <p class="text-sm text-gray-600 mt-1">Isi formulir di bawah untuk menambahkan wahana atau fasilitas baru</p>
    </div>
    <a href="{{ route('admin.facilities.index') }}" class="bg-white border-2 border-gray-300 text-gray-600 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors flex items-center group">
        ...
        <form action="{{ route('admin.facilities.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <!-- Image Option -->
                    <label class="block p-4 bg-white border-2 rounded-xl cursor-pointer hover:border-primary transition-colors group">
                        <div class="flex items-center">
                            <div class="shrink-0">
                            ...
                            <div class="ml-4 flex-grow">
                                <div class="flex items-center">
                                    <input type="radio" name="display_type" value="image"
                                        ...
                                        {{ old('display_type') === 'image' ? 'checked' : '' }}>
                                    <span class="text-sm font-medium text-gray-900">Tampilkan dengan Foto</span>
                                </div>
                                <p class="text-xs text-gray-500 mt-1">Upload foto fasilitas/wahana untuk tampilan yang lebih menarik</p>
                            </div>
                        </div>
                    </label>
                    <!-- Icon Option -->
                            ...
                            <div class="ml-4 flex-grow">
                                <div class="flex items-center">
                                    <input type="radio" name="display_type" value="icon"
                                        class="text-primary focus:ring-primary mr-2"
                                        {{ old('display_type', 'icon') === 'icon' ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <div class="relative">
                        <textarea name="description" id="description" rows="3"
                            class="w-full rounded-lg border-gray-300 focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50 @error('description') border-red-500 @enderror"
                            placeholder="Jelaskan detail tentang wahana atau fasilitas ini">{{ old('description') }}</textarea>
            ...
            <div class="space-y-6">
                ...
                <div class="image-section bg-blue-50 p-4 rounded-lg" style="display: {{ old('display_type') === 'image' ? 'block' : 'none' }}">
                ...
                <!-- Main Icon Section -->
                <div class="main-icon-section bg-blue-50 p-4 rounded-lg" style="display: {{ old('display_type', 'icon') === 'icon' ? 'block' : 'none' }}">
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-2" for="main_icon">
                            Pilih Icon Utama
                            <span class="text-gray-500 text-xs block mt-1">Icon yang akan ditampilkan sebagai tampilan utama</span>
                        </label>
                        <div class="relative">
                            <input type="text" readonly
                                ...
                                value="{{ old('main_icon', 'fas fa-landmark') }}">
                            <input type="hidden" name="main_icon" value="{{ old('main_icon', 'fas fa-landmark') }}">
                            <div class="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none text-gray-500">
                                <i class="fas fa-chevron-down"></i>
                            </div>
                        </div>
                        <p class="text-gray-500 text-xs mt-1">Klik untuk memilih icon yang mewakili fasilitas/wahana ini</p>
                        @error('main_icon')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                </div>
            </div>            <!-- Additional Icon Field (always visible) -->
            <div class="mb-4 bg-yellow-50 p-4 rounded-lg">
                ...
                <div class="relative">
                    <input type="text" readonly
                        ...
                        value="{{ old('icon', 'fas fa-info-circle') }}">
                    <input type="hidden" name="icon" value="{{ old('icon', 'fas fa-info-circle') }}">
                    <div class="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none text-gray-500">
                        <i class="fas fa-chevron-down"></i>
                    </div>
                </div>
                <p class="text-gray-500 text-xs mt-1">Icon ini akan ditampilkan sebagai informasi tambahan di samping nama fasilitas/wahana</p>
                @error('icon')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="type" id="type"
                    ...
                    <option value="wahana" {{ old('type') === 'wahana' ? 'selected' : '' }}>Wahana Permainan/Hiburan</option>
                    <option value="fasilitas" {{ old('type') === 'fasilitas' ? 'selected' : '' }}>Fasilitas Umum</option>
                </select>
